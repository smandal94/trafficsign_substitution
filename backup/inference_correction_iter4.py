...
def main():
    ...
    save_dir = '/media/sayan/sayan/projects/traffic_sign/dataset/DFG_traffic_sign_dataset/results/DFG_resultcrops/iter4'
    ...
    for images, targets, img_paths in tqdm(test_data_loader):
        ...
        for img, target, img_path in zip(images, targets, img_paths):
            ...
            for msk, bxs, lbl, annot_id in zip(masks, boxes, labels, annot_ids):
                ...
                aligned_template, aligned_result = align_superglue(template, final_crop, new, check_cat)
            ...
            # copy_paste = copy_paste[175:-175,:,:]
            # new_mask = new_mask[175:-175,:]
            # lama_img = lama_img[175:-175,:,:]
            # img = img[175:-175,:,:]

            comp = rainnet_transform_image(Image.fromarray(copy_paste[175:-175,:,::-1])).unsqueeze(0).to(device)
            mask = rainnet_transform_mask(Image.fromarray(new_mask[175:-175,:])).unsqueeze(0).to(device)
            real = rainnet_transform_image(Image.fromarray(lama_img[175:-175,:,::-1])).unsqueeze(0).to(device)
            pred = rainnet.processImage(comp, mask, real)

            pred_rgb = util.tensor2im(pred[0:1])[:,:,::-1]
            pred_rgb = cv2.resize(pred_rgb, (800, 450), None, interpolation=cv2.INTER_AREA)
            pred_final = np.zeros_like(img, dtype=np.uint8)
            pred_final[175:-175,:,:] = pred_rgb
            # print(pred_final.shape)


            # all = np.hstack((img, pred_final, copy_paste))
            # cv2.imshow('img', all)
            # # cv2.imshow('new_mask', new_mask)
            ...
            for msk, bxs, lbl, annot_id in zip(masks, boxes, labels, annot_ids):
                ...
                save_pathR = '{}/real/{:06d}.png'.format(save_dir, annot_id)
                save_pathS = '{}/synthetic_rainnet/{:06d}.png'.format(save_dir, annot_id)
                cv2.imwrite(save_pathR, img[ymin:ymax, xmin:xmax, :])
                cv2.imwrite(save_pathS, pred_final[ymin:ymax, xmin:xmax, :])
