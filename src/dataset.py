...
class DFGTrafficSignDataset(torch.utils.data.Dataset):
    def __init__(self, img_dir, split_path, annFile, transforms=None):
        ...
        self.palette = [[168, 140, 21], [81, 147, 215], [120, 96, 195], [175, 140, 37], [17, 218, 132], 
            ...
            [193, 249, 141], [95, 61, 113], [101, 206, 187], [0, 0, 0]]
    ...
    def __getitem__(self, idx):
        ...
        for i in range(len(anns)):
            ...
            # mask = np.maximum(self.coco.annToMask(anns[i])*label, mask)
            ...
            areas.append(area)
            annot_ids.append(int(float(annot_id)))
            
        
        if self.transforms is not None:
            augmented = self.transforms(image=img, masks=masks, bboxes=bboxes, category_id=labels)
            img = augmented['image']
            masks = augmented['masks']
            bboxes = augmented['bboxes']

            # plot_image_anns(img, masks, bboxes)
        ...
        areas = torch.as_tensor(np.array(areas, dtype=np.float32), dtype=torch.float32)
        annot_ids = torch.as_tensor(np.array(annot_ids, dtype=np.int64), dtype=torch.int64)
...
def main():
    ...
    train_transform = A.Compose(
        [
            A.LongestMaxSize(256, interpolation=1),
            A.PadIfNeeded(min_height=256, min_width=256, border_mode=cv2.BORDER_CONSTANT, value=0, mask_value=0, p=1),
