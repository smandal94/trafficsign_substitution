...
def main():
    ...
    # test_annFile = '../../dataset/DFG_traffic_sign_dataset/DFG-tsd-aug-annot-json/test.json'
    # test_split = '../../dataset/DFG_traffic_sign_dataset/splits/test.txt'
    batch_size = 16
    num_workers = 16
    n_classes = 117
    threshold = 0.5
    ...
    for images, targets in tqdm(test_data_loader):
        ...
        for img, target, pred in zip(images, targets, predictions):
            ...
            cv2.imshow('img', np.hstack([gt, result]))
            # cv2.imshow('img', gt)
            key = cv2.waitKey(0)
            if key == 27:
                cv2.destroyAllWindows()
